...
#nmidf = pd.read_html('C:/Users/joeyn/Documents/Indicators/US/ISM Reports/NMI/September NMI.html')
...
print(sht1.name, nmi_val)
val_col={nmi_val:2}
WriteVal(sht1, last_month, val_col)
val_col.clear()#Clears the dictionary for the next function to use the dictionary

#loop to write the sheet names from BA to Inventory
for x in range(1, 11):
    shtx=wb.sheets[x+2]
    x_val = float(nmi.iloc[x,1]) 
    ...
    if shtx.name == 'Exports':
        exports_val = x_val
        val_col={x_val:2, nmi_val:4}
        WriteVal(shtx, last_month, val_col)
        val_col.clear()
    elif shtx.name == 'Imports':
        val_col={x_val:2, exports_val:4, nmi_val:6}
        WriteVal(shtx, last_month, val_col)
        val_col.clear()
    else:
        val_col={x_val:2, nmi_val:4}
...
#time.sleep(3)
#wb.close()
#print(r'ISM_NonManufacturing_Index.xlsx Saved and Closed')
print(r'ISM_NonManufacturing_Index.xlsx Saved')